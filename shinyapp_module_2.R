...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(h4("Predicted Body Fat"),
              textOutput("result"),
              plotOutput("gauge"))#,
  )
)
...
server <- function(input, output) {
  ...
  output$result <- renderText({
    if (input$weight==50 | input$abdomen==40 | input$wrist ==12)
      ...
      paste0("Your body fat percentage is ",round(bodyfat(),3),"%. You are underweight! Please have a nutritious diet and exercise.")
    else if (bodyfat()<17)
      paste0("Your body fat percentage is ",round(bodyfat(),3),"%. You are in the fit range! That's great! Maintain it well.")
    else if (bodyfat()<25)
      paste0("Your body fat percentage is ",round(bodyfat(),3),"%. You have normal body fat % ! Continue your diet and try to exercise more.")
    else if (bodyfat()<45)
      paste0("Your body fat percentage is ",round(bodyfat(),3),"%. You are either overweight or obese! Time to cut down carbs and start exercising.")
    else
